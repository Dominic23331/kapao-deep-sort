...
class Tracker:
    ...
    def update(self, detections, classes, confidences, pose=None):
        ...
        for track_idx, detection_idx in matches:
            self.tracks[track_idx].update(
                self.kf, detections[detection_idx], classes[detection_idx], confidences[detection_idx], pose[detection_idx])
        for track_idx in unmatched_tracks:
            self.tracks[track_idx].mark_missed()
        for detection_idx in unmatched_detections:
            self._initiate_track(detections[detection_idx], classes[detection_idx].item(), confidences[detection_idx].item(), pose[detection_idx])
    ...
    def _initiate_track(self, detection, class_id, conf, pose=None):
        mean, covariance = self.kf.initiate(detection.to_xyah())
        self.tracks.append(Track(
            mean, covariance, self._next_id, class_id, conf, self.n_init, self.max_age,
            detection.feature, pose=pose))
        self._next_id += 1
